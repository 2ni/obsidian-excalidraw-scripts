 ...
 * FIX: Uses view.excalidrawAPI.getSceneElements() to find locked elements
 * FIX: Proper restacking for locked origin markers
 */
...
const createVisuals = async (x, y, id) => {
    ea.clear();
    ea.style.strokeColor = "#e03131"; ea.style.backgroundColor = "#ffc9c9"; ea.style.fillStyle = "solid"; ea.style.strokeWidth = 1; ea.style.roughness = 0; ea.style.fontSize = 16;
    const visualIds = [
        ea.addEllipse(x - 5, y - 5, 10, 10),
        ea.addLine([[x - 10, y], [x + 10, y]]),
        ea.addLine([[x, y - 10], [x, y + 10]]),
